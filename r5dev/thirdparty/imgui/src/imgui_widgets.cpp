...
#include "thirdparty/imgui/include/imstb_textedit.h"
...
bool ImGui::BeginMenuIcon(ImTextureID texId, const char* label, const char* icon, bool enabled, const ImVec2& imageSize, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const bool hasText = label_size.x > 0;
    const float innerSpacing = hasText ? ((frame_padding >= 0) ? (float)frame_padding : (style.ItemInnerSpacing.x)) : 0.f;
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float)frame_padding, (float)frame_padding) : style.FramePadding;
    const ImVec2 totalSizeWithoutPadding(size.x + innerSpacing + label_size.x, size.y > label_size.y ? size.y : label_size.y);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + totalSizeWithoutPadding + padding * 2);
    ImVec2 start(0, 0);
    start = window->DC.CursorPos + padding; if (size.y < label_size.y) start.y += (label_size.y - size.y) * .5f;
    const ImRect image_bb(start, start + size);
    start = window->DC.CursorPos + padding; start.x += size.x + innerSpacing; if (size.y > label_size.y) start.y += (size.y - label_size.y) * .5f;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        float w = totalSizeWithoutPadding.x;
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = Selectable("", menu_is_open, selectable_flags, ImVec2(w, 0.0f));
...
namespace ImGui
{
    ...
    //Custom
    bool ImageButtonWithText(ImTextureID texId, const char* label, bool showcheck, const ImVec2& imageSize, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col) {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
            return false;
        ...
        const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + totalSizeWithoutPadding + padding * 2);
        const ImRect bb2(window->DC.CursorPos, window->DC.CursorPos + totalSizeWithoutPadding + padding * 2 - ImVec2(imageSize.x, 0));
        ImVec2 start(0, 0);
        ImVec2 ogstart(0, 0);
        start = window->DC.CursorPos + padding; if (size.y < textSize.y) start.y += (textSize.y - size.y) * .5f;
        ogstart = window->DC.CursorPos + padding; if (size.y < textSize.y) start.y += (textSize.y - size.y) * .5f;
        const ImRect image_bb(start - ImVec2(0, 5), start + size);
        ...
        if (showcheck)
            ...
            pressed = ButtonBehavior(bb2, id, &hovered, &held);

        // Render
        const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);

        ImVec2 testmax(bb.Max.x - imageSize.x, bb.Max.y);

        if (showcheck)
            RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, style.FrameRounding));
        else
            RenderFrame(bb.Min, testmax, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, style.FrameRounding));
        ...
        if (textSize.x > 0) {
            if(showcheck)
                ...
                ImGui::RenderText(ogstart, label);
        }

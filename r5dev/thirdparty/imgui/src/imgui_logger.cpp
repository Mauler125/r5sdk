...
void CTextLogger::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
...
void CTextLogger::SetPalette(const Palette& aValue)
{
...
int CTextLogger::InsertTextAt(Coordinates& /* inout */ aWhere, const char* aValue)
{
	assert(!m_bReadOnly);
...
void CTextLogger::AddUndo(UndoRecord& aValue)
{
	assert(!m_bReadOnly);
...
void CTextLogger::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
...
void CTextLogger::SetText(const std::string& aText)
{
	m_Lines.clear();
...
void CTextLogger::SetTextLines(const std::vector<std::string>& aLines)
{
	m_Lines.clear();
	...
	if (HasSelection())
	{
		if (aChar == '\t' && m_State.m_SelectionStart.m_nLine != m_State.m_SelectionEnd.m_nLine)
		{
			...
			for (int i = start.m_nLine; i <= end.m_nLine; i++)
			{
				...
				{
					line.insert(line.begin(), Glyph('\t', CTextLogger::PaletteIndex::Background));
					modified = true;
				}
			}
...
void CTextLogger::SetColorizerEnable(bool aValue)
{
...
void CTextLogger::InsertText(const std::string& aValue)
{
	InsertText(aValue.c_str());
}

void CTextLogger::InsertText(const char* aValue)
{
	if (aValue == nullptr)
		return;
...
void CTextLogger::DeleteSelection()
{
...
bool CTextLogger::CanUndo() const
{
	return !m_bReadOnly && m_nUndoIndex > 0;
}

bool CTextLogger::CanRedo() const
{
	return !m_bReadOnly && m_nUndoIndex < (int)m_UndoBuffer.size();
}

void CTextLogger::Undo(int aSteps)
{
	while (CanUndo() && aSteps-- > 0)
		m_UndoBuffer[--m_nUndoIndex].Undo(this);
}

void CTextLogger::Redo(int aSteps)
{
	while (CanRedo() && aSteps-- > 0)
		m_UndoBuffer[m_nUndoIndex++].Redo(this);
}

const CTextLogger::Palette& CTextLogger::GetDarkPalette()
{
...
const CTextLogger::Palette& CTextLogger::GetLightPalette()
{
	const static Palette p = { {
			0xff7f7f7f,	// None
			0xffff0c06,	// Keyword	
...
const CTextLogger::Palette& CTextLogger::GetRetroBluePalette()
{
	const static Palette p = { {
			0xff00ffff,	// None
			0xffffff00,	// Keyword	
...
void CTextLogger::ProcessInputs()
{
}

void CTextLogger::Colorize(int aFromLine, int aLines)
{
...
void CTextLogger::ColorizeRange(int aFromLine, int aToLine)
{
...
void CTextLogger::ColorizeInternal()
{
...
CTextLogger::UndoRecord::UndoRecord(
	const std::string& aAdded,
	const CTextLogger::Coordinates aAddedStart,
	const CTextLogger::Coordinates aAddedEnd,
	const std::string& aRemoved,
	const CTextLogger::Coordinates aRemovedStart,
	const CTextLogger::Coordinates aRemovedEnd,
	CTextLogger::EditorState& aBefore,
	CTextLogger::EditorState& aAfter)
	: m_svAdded(aAdded)
...
void CTextLogger::UndoRecord::Undo(CTextLogger* aEditor)
{
	if (!m_svAdded.empty())
	{
...
void CTextLogger::UndoRecord::Redo(CTextLogger* aEditor)
{
	if (!m_svRemoved.empty())
	{
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::CPlusPlus()
{
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::HLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::GLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::C()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::SQL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::AngelScript()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const CTextLogger::LanguageDefinition& CTextLogger::LanguageDefinition::Lua()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)

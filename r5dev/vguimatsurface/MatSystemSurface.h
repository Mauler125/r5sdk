...
class VMatSystemSurface : public IDetour
{
	...
	{
		p_CMatSystemSurface_DrawColoredText = g_GameDll.FindPatternSIMD(reinterpret_cast<rsig_t>("\x4C\x8B\xDC\x48\x83\xEC\x68\x49\x8D\x43\x58\x0F\x57\xC0"), "xxxxxxxxxxxxxx");
	...
	virtual void GetVar(void) const
	{
		...
		g_pMatSystemSurface = g_GameDll.FindPatternSIMD(reinterpret_cast<rsig_t>(
			...
			"xxx?????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????xxx????").ResolveRelativeAddressSelf(0x3, 0x8).RCast<CMatSystemSurface*>();
